...
# Refactored for improved robustness and modularity.
...
def rasterize(pptx_path: str, out_dir: str, dpi: int, soffice_path: str) -> Sequence[str]:
...
def check_overflow(pptx_path: str, dpi: int, soffice_path: str) -> Dict[str, Any]:
    """
    Checks input PPTX for edge-overflowing content and returns a dictionary
    with results instead of printing to stdout.
    """
    ...
    img_paths = rasterize(enlarged_pptx, img_dir, dpi, soffice_path)
    failing_indices = inspect_images(img_paths, pad_ratio_w, pad_ratio_h, dpi)

    failing_slides = {
        "indices": failing_indices,
        "image_paths": [img_paths[i - 1] for i in failing_indices]
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=str, default="ppt-preview", help="Output directory for the rendered PNGs (default: ppt-preview)")
    parser.add_argument("--width", type=int, default=1600, help="Approximate maximum width in pixels after isotropic scaling (default 1600).")
    parser.add_argument("--height", type=int, default=900, help="Approximate maximum height in pixels after isotropic scaling (default 900).")
    # --- IMPROVEMENT: Make soffice path configurable ---
    parser.add_argument("--soffice-path", type=str, default="soffice", help="Path to the LibreOffice soffice binary.")
    ...
    # --- REFACTOR: Call refactored overflow check and handle its output ---
    overflow_results = check_overflow(args.input, dpi, args.soffice_path)
    failing_slides = overflow_results["failing_slides"]

    if failing_slides["indices"]:
        print(
            "WARNING: Slides with content overflowing original canvas (1-based indexing): "
            + ", ".join(map(str, failing_slides["indices"]))
            + "\n"
            + "    Rendered images with grey paddings for problematic slides are available at: "
        )
        for path in failing_slides["image_paths"]:
            print("   ", path)
        print(
    ...
    rasterize(args.input, out_dir, dpi, args.soffice_path)

...
use crate::{
    ...
    auth::{AuthManager, Claims, LoginRequest, LoginResponse, auth_middleware, extract_token},
    middleware::{
...
/// Logout endpoint to revoke current JWT
#[instrument(skip(state, headers))]
async fn logout(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> Result<StatusCode, StatusCode> {
    let token = extract_token(&headers).map_err(|_| StatusCode::UNAUTHORIZED)?;
    state
        .auth_manager
        .revoke_token(&token)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn serve(settings: &Settings) -> Result<()> {
    ...
    let auth_manager = Arc::new(AuthManager::new(jwt_secret, &db_path, &settings.security).await?);
...
fn get_jwt_secret_for_server(settings: &Settings) -> Result<Secret<String>> {
